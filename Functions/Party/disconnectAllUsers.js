  ...
  try {
    ...
    for (const userId of validUserIds) {
      ...
      if (member.voice.channel !== null && (member.voice.channelId === CHANNELS.END || member.voice.channelId === CHANNELS.TEAM1 || member.voice.channelId === CHANNELS.TEAM2)) {
        ...
        array.push(`${EMOJIS.sageGood} - User disconnected > ${member}`);
      } else {
        array.push(`${EMOJIS.sageCry} - User was already disconnected > ${member}`);
      }

      await new Promise(resolve => setTimeout(resolve, 500)).catch(O_o => { console.log(O_o) });
    }

    guild.channels.cache.get(IDS.CHANNELS.LOG).send({ embeds: [await createEmbed.log(interaction.member, `### ${EMOJIS.sagePeek} | LOGS - Users disconneted\n> ${array.join("\n> ")}`)] });
