...
module.exports = class RankCard {
  constructor() {
    this.backgroundImage = `${process.cwd()}/functions/api/canvas-AngelBot/assets/img/background.png`;
    ...
    this.colorLevelBox = "#8015EA";
    this.colorReputationBox = "#8015EA";
  ...
  async toAttachment() {
    ...
    ctx.font = applyText(canvas, lvlText, 32, 170, this.font);
    ...
    ctx.font = applyText(canvas, lvlText, 32, 170, this.font);
    ctx.textAlign = "center";
    ctx.fillText(lvlText, 50 + 30 + 180 / 2, 30 + 180 + 30 + 30 + 50 + 38);
    ...
    //   ctx.font = applyText(canvas, repText, 32, 170, this.font);
    //   ctx.textAlign = "center";
    ...
    ctx.font = applyText(canvas, this.username, 60, 460, this.font);
    ...
    if (this.addonRank) {
      ...
      ctx.font = applyText(canvas, "#" + this.rank, 45, 194, this.font);
      ctx.fillText("#" + this.rank, canvas.width - 50 - 5, 80);
    }
    if (this.addonRankName) {
      ctx.textAlign = "left";
      ctx.fillStyle = this.colorRankName;
      ctx.font = applyText(canvas, this.rankName, 35, 690, this.font);
    ...
    if (this.addonBadges) {
      ...
      for(let index = 0; index < badgeNames.length; index++){
        ...
        if (!this[badge]){
          ...
          ctx.textAlign = "center";
          ctx.font = applyText(canvas, ".", 120, 50, this.font);
    ...
    // ctx.font = applyText(canvas, textXPEdited, 25, 690, this.font);
    // ctx.fillText(
    //   textXPEdited,
    //   50 + 240 + 45 + 5 + 690 / 2,
    //   80 + 45 + 15 + 30 + 90
